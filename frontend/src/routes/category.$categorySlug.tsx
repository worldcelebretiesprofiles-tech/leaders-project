...
export const Route = createFileRoute("/category/$categorySlug")({
  loader: async ({ params }) => {
    try {
      ...
      const profiles = currentCategory
        ? await getProfiles({ category_id: currentCategory.id })
        : [];
      return { profiles, categories };
    } catch (err) {
      console.error("Failed to load category page data in loader:", err);
      return { profiles: [], categories: [] };
    }
  },
  head: ({ loaderData, params }) => {
    ...
    const title = currentCategory
      ? `${currentCategory.name} — Verified Leadership Directory | Global Leader Sphere`
      : "Directory Sector — Global Leader Sphere";
    const description = currentCategory
      ? `Explore verified changemakers, delegates, and community leaders listed under the sector of ${currentCategory.name}.`
...
function CategoryDirectory() {
  ...
  const filteredProfiles = useMemo(() => {
    ...
    const categoryProfiles = profiles.filter((p: any) => p.category_id === currentCategory.id);
    
    const query = searchTerm.toLowerCase().trim();
    if (!query) return categoryProfiles;
  ...
  }, [profiles, currentCategory, searchTerm]);
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-midnight text-foreground px-4 text-center">
        ...
        <div className="max-w-md glass-strong p-8 rounded-3xl border-white/10 shadow-2xl relative z-10">
          ...
          <h1 className="text-3xl font-display font-bold text-sky mb-3">Category Not Found</h1>
          <p className="text-foreground/75 mb-8">
            The requested category directory does not exist or has been modified.
          </p>
          <Link
            to="/"
  ...
  return (
    <div className="relative min-h-screen text-foreground bg-midnight font-sans">
                          ...
                          <div className="pl-3 border-l border-white/5 space-y-1">
                            {cat.subcategories.map((sub: any) => (
                              <Link
                                ...
                                className="block text-[11px] text-foreground/60 hover:text-sky transition"
                              >
                                {sub.name}
                              </Link>
                            ))}
      ...
      {/* ========================================================================= */}
      {/* CATEGORY TITLE BANNER SECTION                                             */}
      {/* ========================================================================= */}
      <section className="max-w-7xl mx-auto pt-12 pb-6 px-6 md:px-12 z-10 relative">
        <div className="space-y-6">
          <Link
            to="/"
            className="inline-flex items-center gap-1.5 text-xs text-foreground/50 hover:text-sky transition font-semibold"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/5">
            <div>
              <ScrollReveal animation="fade-up" delay={0}>
                <div className="inline-flex items-center gap-2 chip py-1.5 px-4 bg-sky/10 border-sky/25 text-sky font-bold text-[11px] uppercase tracking-widest mb-3">
                  <Sparkles className="size-3.5" /> Verified Directory Listing
                </div>
              </ScrollReveal>
              ...
              <ScrollReveal animation="fade-up" delay={300}>
                <p className="text-sm text-foreground/60 leading-relaxed font-sans max-w-2xl mt-3">
                  Explore verified chagemakers, delegates, and community leaders running initiatives 
                  under the category of {currentCategory.name}.
                </p>
              </ScrollReveal>
            </div>
            ...
            <div className="text-xs text-foreground/40 font-semibold shrink-0">
              ...
              <span className="text-sky font-bold font-mono text-base">
        ...
        <div className="glass-strong rounded-3xl p-6 border-white/10 shadow-xl flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              placeholder="Search category leaders..."
            />
            {searchTerm && (
              <button
        ...
        {filteredProfiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[300px] gap-4 glass rounded-3xl p-12 border-white/5 text-center max-w-md mx-auto">
            ...
            <p className="text-xs text-foreground/60 leading-relaxed">
              We couldn't find any verified leader listed in this directory category matching your selection.
            </p>
            {searchTerm && (
              <button
                onClick={() => setSearchTerm("")}
